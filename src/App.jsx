...
class Topic {
    ...
    getOverdueAmount(currentLesson) {
        return (currentLesson - this.lastReviewedLesson) - this.interval;
    }
}
...
export default function App() {
    ...
    const generateSchedule = () => {
        ...
        lessons.forEach(lesson => {
            ...
            let dueTopics = allTopics.filter(topic => topic.isDueForReview(lesson.lessonNumber));

            // Prioritize overdue topics (most overdue first)
            dueTopics.sort((a, b) => b.getOverdueAmount(lesson.lessonNumber) - a.getOverdueAmount(lesson.lessonNumber));

            // Select a maximum of 2 topics to be recalled
            dueTopics = dueTopics.slice(0, 2);

            // Update recall status for the selected topics
            dueTopics.forEach(topic => topic.updateRecall(lesson.lessonNumber, 3));

            // Store the lesson schedule
